const decrypt = (string) => {
    ...
    const arrayStrings = words.split(' ')
    let decrypt = ''
    arrayStrings.map(word => {
        const string = word + searchRepeatWordInArray(arrayStrings, word)
        if (!decrypt.includes(string)) {
...
const searchRepeatWordInArray = (array, string) => {
    let count = 0
    array.forEach(word => {
        if (word === string) {
            count++
